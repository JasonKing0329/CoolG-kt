...
class MatchViewModel(application: Application): BaseViewModel(application) {
    ...
    private var countComposite = CompositeDisposable()
    private var roundComposite = CompositeDisposable()

    fun loadItems() {
        // stop round or count async loading
        roundComposite.clear()
        countComposite.clear()
    ...
    fun loadCount() {
        ...
        BasicAndTimeWaste<Any>()
            ...
            .subscribe(
                ...
                object : SimpleObserver<TimeWasteRange>(countComposite) {
                    override fun onNext(t: TimeWasteRange) {
                        countItemsRange.value = t
                    }
    ...
    private fun loadRecords(): Observable<List<Any>> {
        return Observable.create {
    ...
    private fun countTimeWaste(): TimeWasteTask<Any> {
    ...
    fun loadRoundItems() {
        // stop count async loading
        countComposite.clear()

        loadingObserver.value = true
        BasicAndTimeWaste<Any>()
            .basic(loadRoundRecords())
            .timeWaste(roundTimeWaste(), 20)
            .composite(getComposite())
            .subscribe(
                object : SimpleObserver<List<Any>>(getComposite()) {
                    override fun onNext(t: List<Any>) {
                        loadingObserver.value = false
                        roundItemsObserver.value = t
                    }
                ...
                object : SimpleObserver<TimeWasteRange>(roundComposite) {
                    override fun onNext(t: TimeWasteRange) {
                        roundItemsRange.value = t
                    }
    ...
    private fun loadRoundRecords(): Observable<List<Any>> {
        return Observable.create {
            val list = mutableListOf<Any>()
            // 只统计R16及以上的轮次
    ...
    private fun roundTimeWaste(): TimeWasteTask<Any> {
        return object : TimeWasteTask<Any> {
            override fun handle(index: Int, data: Any) {
